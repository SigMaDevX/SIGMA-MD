...
// Fixed global utility functions
global.__filename = function(filename = import.meta.url) {
  return filename.startsWith('file://') ? fileURLToPath(filename) : filename;
};

global.__dirname = function(filename = import.meta.url) {
  return dirname(global.__filename(filename));
};

global.__require = function(filename = import.meta.url) {
...
async function main() {
  ...
  try {
    ...
    console.log("SigmaSession completed successfully ✅");
  } catch (error) {
    console.error("Error in SigmaSession:", error);
    process.exit(1);
  }
}
